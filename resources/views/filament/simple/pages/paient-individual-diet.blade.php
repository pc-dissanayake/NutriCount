<x-filament-panels::page>
    ...
    <nav class="text-sm  bg-gray-100 dark:bg-gray-800 p-3 rounded-full">
        ...
        <span class="mx-2">&gt;</span>
        <span class="text-gray-500 dark:text-gray-400">Patient Diet Entry</span>
    </nav>

   @if ($date && request('unit_id'))
    <div class="mt-6 bg-white dark:bg-gray-900 shadow-sm rounded-xl p-6 space-y-6 border border-gray-200 dark:border-gray-700" id="selected-date-unit">
        ...
        @php
            ...
            $patientDiets = collect();
            if ($selectedPatient) {
                $patientDiets = \App\Models\HospitalUnitDietAmount::where('patient_id', $selectedPatient->id)->get();
            }
        @endphp
        ...
        <div class="text-sm text-gray-700 dark:text-gray-300">
            ...
            @if ($selectedPatient)
                <ul class="mt-2 space-y-1 list-disc list-inside">
                    <li><strong>Name:</strong> {{ $selectedPatient->name ?? $selectedPatient->full_name ?? 'N/A' }}</li>
                    <li><strong>Gender:</strong> {{ $selectedPatient->gender ?? 'N/A' }}</li>
                    <li><strong>DOB:</strong> {{ $selectedPatient->date_of_birth ?? 'N/A' }}</li>
                    <li><strong>BHT:</strong> {{ $selectedPatient->bht ?? 'N/A' }}</li>
                    <li><strong>PHN:</strong> {{ $selectedPatient->phn ?? 'N/A' }}</li>
                </ul>
            @else
                <p class="mt-2 text-red-500 dark:text-red-400">No patient selected.</p>
            @endif
        </div>
        ...
        <div>
            <button
                type="button"
                id="show-form"
                class="filament-button filament-button-size-sm filament-button-color-primary bg-primary-600 hover:bg-primary-700 text-white font-medium px-4 py-2 rounded-lg"
            >
                Edit
            </button>
        </div>
...
@endif
    ...
    <form method="GET" action="{{ url('/simple/paient-individual-diet') }}" class="space-y-4 bg-gray-100 dark:bg-gray-800 p-4 rounded-xl shadow">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="flex flex-col">
                <label for="date" class="font-semibold mb-1 text-gray-900 dark:text-gray-100">Date:</label>
                <input type="date" id="date" name="date" value="{{ $date ?? '' }}" class="border rounded-xl px-3 py-2 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-primary-500" readonly>
            </div>
            <div class="flex flex-col">
                <label for="unit_id" class="font-semibold mb-1 text-gray-900 dark:text-gray-100">Unit:</label>
                <select id="unit_id" name="unit_id" class="border rounded-xl px-3 py-2 w-full bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-primary-500" readonly>
            ...
            <div class="flex flex-col">
                <label for="patient_id" class="font-semibold mb-1 text-gray-900 dark:text-gray-100">Patient:</label>
                <select id="patient_id" name="patient_id" class="border rounded-xl px-3 py-2 w-full bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-primary-500" required>
                    <option value="" disabled selected>Select a patient</option>
                    @foreach ($patients as $patient)
                        <option value="{{ $patient->id }}" {{ request('patient_id') == $patient->id ? 'selected' : '' }}>{{ $patient->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <button type="submit" class="filament-button filament-button-primary bg-primary-500 text-white hover:bg-primary-600 focus:ring-primary-500 px-4 py-2 m-2 rounded-xl">Change Patient</button>
        </div>
    </form>
</div>
    ...
    @if (!empty($simpleDiets))
        <div class="mt-8 bg-gray-100 dark:bg-gray-900 p-3 rounded-xl">
            ...
            <form method="POST" action="{{ url('/simple/paient-individual-diet/save') }}" class="space-y-4">
                @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Ensure the form container is hidden on page load
            document.getElementById('form-container').classList.add('hidden');

            // Add event listener to the 'Edit' button
            document.getElementById('show-form').addEventListener('click', function () {
                document.getElementById('form-container').classList.toggle('hidden');
                document.getElementById('selected-date-unit').classList.toggle('hidden');
                document.querySelector('.mt-8').classList.toggle('hidden');
            });
        });
    </script>
</x-filament-panels::page>
